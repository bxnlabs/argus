 ...
 * catches up: `sessionsLoaded` tracks whether the list query holds data at all,
 * which is already true for a cached list with a refetch in flight, so a create
 * or clone handed over from another node lands while that node's cached list
 * still predates it.
