...
import { UnreadBadge } from "@/components/NodeRail/UnreadBadge";
 ...
 * a top-right badge summing the unread counts waiting on the *other* online nodes
 * — what you can't see with the rail closed; once it's open those counts live on
 * the individual rail tiles instead. With no active node (empty/errored registry)
 * it falls back to a Plus tile so the rail stays reachable.
 */
export function NodeStatus({
  ...
  // Unread waiting on the *other* online nodes — the active node is excluded
  // (you're already looking at it, just like the rail tiles never badge it).
  const otherUnread = nodes.reduce(
    (sum, n) =>
      n.id !== activeNode.id && n.online ? sum + (n.summary?.attention ?? 0) : sum,
    0,
  );
      ...
      <TooltipTrigger asChild>
        ...
        >
          ...
          {!railOpen && otherUnread > 0 && (
            <UnreadBadge count={otherUnread} data-testid="node-status-attention" />
          )}
        </button>
      </TooltipTrigger>
