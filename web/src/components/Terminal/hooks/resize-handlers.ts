...
export function setupResizeHandlers(config: ResizeHandlersConfig): () => void {
  ...
  // Sending the same cols/rows causes tmux SIGWINCH → full redraw
  // even though nothing actually changed (the "glitch").
  let lastSentCols = 0;
  let lastSentRows = 0;
  const deduplicatedResize = (cols: number, rows: number) => {
  ...
  let fitTimeouts: ReturnType<typeof setTimeout>[] = [];
  ...
  const doFit = () => {
    // Skip when container is hidden (display:none) to avoid resizing
    // tmux to tiny dimensions while a panel overlay is active.
    ...
    requestAnimationFrame(() => {
      ...
      restoreScroll();
      deduplicatedResize(term.cols, term.rows);
