...
import { useCommitDetailQuery } from "@/data/git";
import type { CommitSummary, CommitFile, FileStatus } from "@/types";
...
export function CommitItem({
  ...
  const { data: detail, isLoading } = useCommitDetailQuery(
    workingDir,
  ...
  return (
    <div className="border-border/30 border-b last:border-b-0">
      <button
        onClick={onToggle}
        className={cn(
          "hover:bg-muted/50 flex w-full items-center gap-3 px-3 py-1.5 text-left transition-colors",
      ...
      {expanded && (
        <div className="pr-3 py-2 pl-14">
          <p className="mb-1 text-xs font-semibold">{commit.subject}</p>
          {commit.body && (
            <p className="text-muted-foreground mb-3 whitespace-pre-wrap text-xs">
              {commit.body}
            </p>
          )}
          {isLoading ? (
