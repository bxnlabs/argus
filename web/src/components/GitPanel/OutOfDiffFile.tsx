...
// Renders the comment's stored snippet using the same row structure as a real
// diff line (two line-number gutters, marker column, content column) so a
// degraded card visually aligns with rendered hunks elsewhere on the page.
// The line-number gutter shows the comment's anchor line on the side the
// comment was authored against — that line may no longer exist at the current
// ref, but the number is the snapshot the comment was made against, which is
// the same contract real diff hunks honor.
function SnippetDiffRow({ comment }: { comment: ReviewComment }) {
  ...
  // defensively so any legacy multi-line snippet still renders cleanly with
  // each line as its own diff row.
